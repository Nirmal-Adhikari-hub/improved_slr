...
######### DEBUGGING FUNCTIONS #########
def debug_print_alignment(gt, pred1, pred2=None):
    print("\n🔍 WER Debug — Sample Comparison")
    print("📘 Ground Truth Glosses:", gt)
    print("🤖 Predicted (LSTM) Glosses:", pred1)
    if pred2:
        print("🔁 Predicted (Conv) Glosses:", pred2)
    print("-" * 60)
    

def filter_stm_file(original_stm, filtered_stm, valid_ids):
    with open(original_stm, 'r') as infile, open(filtered_stm, 'w') as outfile:
        for line in infile:
...
def wer_calculation(gt_path, primary_pred, auxiliary_pred=None):
    ...
    if auxiliary_pred is not None:
        ...
        for fileid, sent in gt.items():
            debug_print_alignment(sent, pred1[fileid], pred2[fileid])
        ...
        for fileid, sent in gt.items():
            debug_print_alignment(sent, pred1[fileid])
            sent_stat = sent_evaluation(
                info=fileid, gt=sent,
                merge_same=True,
                lstm_prediction=pred1[fileid],
